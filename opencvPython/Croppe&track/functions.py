...
def getPen():
    ...
    global Pen, frame, refPt,ret, selected, trackWindow, PenHist
    clone = frame.copy()
...
def trackUpdate():
    global PenHist, trackWindow, ret, frame, mask
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    mask = cv2.calcBackProject([hsv],[0], PenHist,[0,180],1)
    cv2.imshow('mask', mask)
    ret, trackWindow = cv2.CamShift(mask, trackWindow, term_crit)

def drawIt():
    global frame, ret, selected, trackWindow, mask
    if selected:
        x, y, w, h = trackWindow
